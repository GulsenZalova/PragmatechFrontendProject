let names = ['Eli', 'Ehmed', 'Sara', 'Sevda', 'Qurban'];
let surnames = ['Piriyev', 'Memmedov', 'Qurbanova', 'Ezizova', 'Sadiqzade'];
let ages = [23, 34, 40, 58, 60];
let students = {
    adlar: names,
    soyadlar: surnames,
    yaslar: ages
}

//expected output
// Ad: Eli,Soyad:Piriyev,Yas:23
// Ad: Ehmed,Soyad:Memmedov,Yas:34

console.log("Ad: " + students["adlar"][0] + ", Soyad: " + students["soyadlar"][0] +", Yaş: " + students["yaslar"][0] )
console.log("Ad: " + students["adlar"][1] + ", Soyad: " + students["soyadlar"][1] +", Yaş: " + students["yaslar"][1] )